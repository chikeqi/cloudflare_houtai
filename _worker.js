// Cloudflare Pages 完整版 - 旭儿导航（后台与 EdgeOne 版本一致）
export default {
...
.form-group input,.form-group textarea{width:100%;padding:9px 12px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px}
button{border:none;padding:9px 18px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:600}
...
td{padding:10px 12px;border-bottom:1px solid #f0f2f5;color:#2d3748}
...
.modal-content{background:white;border-radius:14px;padding:28px;width:90%;max-width:560px}
...
<div class="container">
    <div class="header">
        <h1>📚 管理后台</h1>
        <div>
            <button id="changePwdBtn" class="btn-warning" style="background:#ed8936;color:white">🔑 修改密码</button>
            <a href="/logout" style="background:rgba(255,255,255,0.2);color:white;padding:9px 18px;border-radius:8px;text-decoration:none;margin-left:10px">退出登录</a>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="grid-top">
            <div>
                ...
                <div class="form-group"><label>🇨🇳 国内线路链接</label><input type="url" id="cnLink" value="${escapeHtml(cnLink)}" placeholder="https://你的国内备用域名.com"></div>
            </div>
            <div>
                <div class="form-group"><label>Logo URL</label><div style="display:flex;gap:10px"><input type="url" id="logoUrl" value="${escapeHtml(siteLogo)}" style="flex:1"><button id="uploadLogoBtn" class="btn-warning">上传图片</button></div></div>
                <div class="form-group"><label>Logo 跳转链接</label><input type="url" id="logoLink" value="${escapeHtml(siteLogoLink)}"></div>
                <div class="form-group"><label>页眉背景图 URL</label><div style="display:flex;gap:10px"><input type="url" id="headerBgUrl" value="${escapeHtml(headerBg)}" style="flex:1"><button id="uploadHeaderBgBtn" class="btn-warning">上传图片</button></div></div>
            </div>
        </div>
...
<div id="postModal" class="modal">
    <div class="blog-modal-content">
        ...
        <div class="form-actions"><button id="cancelPostBtn" class="btn-secondary">取消</button><button id="savePostBtn" class="btn-success">保存</button></div>
    </div>
</div>
...
<div id="changePwdModal" class="modal">
    <div class="modal-content">
        ...
        <div class="form-actions"><button id="cancelPwdBtn" class="btn-secondary">取消</button><button id="confirmPwdBtn" class="btn-primary">确认修改</button></div>
    </div>
</div>
...
function renderPosts(){
    ...
    document.getElementById('postsList').innerHTML = filtered.map(p => `
        <tr>
            <td>${p.id}</td>
            <td><strong>${escape(p.title)}</strong>${p.pinned?'<span style="background:#fef3c7;color:#92400e;padding:2px 6px;border-radius:12px;font-size:11px;margin-left:6px">📌置顶</span>':''}</td>
            <td>${escape(p.category||'未分类')}</td>
            <td><span style="padding:2px 8px;border-radius:20px;font-size:12px;background:${p.status==='published'?'#d4edda':'#fff3cd'};color:${p.status==='published'?'#155724':'#856404'}">${p.status==='published'?'已发布':'草稿'}</span></td>
            <td>${new Date(p.createdAt).toLocaleDateString()}</td>
            <td class="actions"><button class="btn-warning" onclick="editPost(${p.id})">编辑</button><button class="btn-danger" onclick="deletePost(${p.id})">删除</button></td>
        </table>
    `).join('');
}

function renderSites(){
    document.getElementById('sitesList').innerHTML = allSites.map(s => `
        <tr>
            <td>${s.id}</td>
            ...
            <td class="actions"><button class="btn-danger" onclick="deleteSite(${s.id})">删除</button></td>
        </tr>
    `).join('');
}
