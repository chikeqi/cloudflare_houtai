// Cloudflare Pages Worker - 旭儿导航完整版（修复封面图显示）
export default {
...
function renderPosts(){let sorted=[...allPosts];sorted.sort((a,b)=>{if(a.pinned&&!b.pinned)return-1;if(!a.pinned&&b.pinned)return1;return b.id-a.id;});let html='';for(let p of sorted){html+='<tr><td>'+p.id+'</span></span><tr>。<strong>'+escape(p.title)+'</strong>'+(p.pinned?'<span class="pin-badge">📌置顶</span>':'')+'</span></span></tr>。<span>'+escape(p.category||'未分类')+'</span></span></td>。<span class="status-badge '+(p.status==='published'?'status-published':'status-draft')+'">'+(p.status==='published'?'已发布':'草稿')+'</span></td>。<span>'+new Date(p.createdAt).toLocaleDateString()+'</span></span></td><td class="actions"><button class="btn-warning" onclick="editPost('+p.id+')">编辑</button><button class="btn-danger" onclick="deletePost('+p.id+')">删除</button></span></span></tr>';}document.getElementById('postsList').innerHTML=html;}
function renderSites(){let html='';for(let s of allSites){html+='<tr><td>'+s.id+'</span></span><td><strong>'+escape(s.name)+'</strong></span></span><td><a href="'+escape(s.url)+'" target="_blank">'+escape(s.url).substring(0,50)+'</a></span></span><td>'+escape(s.catelog)+'</span></span><td>'+(s.sort_order||9999)+'</span></span><td><td class="actions"><button class="btn-danger" onclick="deleteSite('+s.id+')">删除</button></span></span></tr>';}document.getElementById('sitesList').innerHTML=html;}
